...
#nome, nome_contato, telefone, endereco, segmento
...
class Application:
    ...
    def __init__(self, parent):
        ...
        #TITULO
        self.titulo = Label(text="Dados do Fornecedor", pady=10)
        ...
        self.nValorAtual = databaseConnect.number_reg('fornecedores') + 1
        ...
        #NOME_CONTATO
        self.nomeLabel = Label(text="Nome Contato", font=self.fontePadrao) 
        self.nomeLabel.grid(row=3, sticky=E)

        self.nome_contato = Entry()
        self.nome_contato["width"] = 30
        self.nome_contato["font"] = self.fontePadrao
        self.nome_contato.grid(row=3, column=1, sticky=W)        


        #TELEFONE
        self.telefoneLabel = Label(text="Telefone", font=self.fontePadrao)
        self.telefoneLabel.grid(row=4, sticky=E)
        ...
        self.enderecoLabel.grid(row=5, sticky=E)
        ...
        self.segmentoLabel.grid(row=6, sticky=E)

        self.segmento = Entry()
        self.segmento["width"] = 30
        self.segmento["font"] = self.fontePadrao
        self.segmento.grid(row=6, column=1, sticky=W)        
    ...
    def insert(self):
        try:
            nome = self.nome.get()
            nome_contato = self.nome_contato.get()
            telefone = self.telefone.get()
            endereco = self.endereco.get()
            segmento = self.segmento.get()
            databaseConnect.insert_fornecedores(nome, nome_contato, telefone, endereco, segmento)
            self.mensagem["text"] = "Cadastrado"
            #Limpar campos
            self.nome.delete(0, END)
            self.nome_contato.delete(0, END)
